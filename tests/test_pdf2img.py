...
class TestPDF2Image:
    ...
    def test_pdf_to_images_function(self, pdf_path, tmp_path):
        ...
        assert all(os.path.exists(img) for img in result), "All image files should exist"
        assert all(img.endswith('.jpg') for img in result), "Default format should be jpg"
        ...
        assert all(os.path.exists(img) for img in result_png), "All image files should exist"
    ...
    def test_pdf2img_progress_callback(self, pdf_path, tmp_path):
        ...
        assert progress_values[-1] == num_pages, "Last progress value should equal page count"