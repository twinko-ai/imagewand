...
import cv2
import numpy as np
from imagewand.align import align_image, _detect_angle_hough, _detect_angle_contour, _detect_angle_center
...
def test_align_image_exists():
    ...
    assert os.path.exists(OCTOPUS_TILT_PATH), f"Test image not found: {OCTOPUS_TILT_PATH}"


def test_align_image_basic():
    """Test basic alignment functionality"""
    # Create a temporary output path
    output_path = os.path.join(os.path.dirname(OCTOPUS_TILT_PATH), "octopus_aligned.jpg")
...
def test_align_image_methods():
    ...
    for method in methods:
        # Create a temporary output path
        output_path = os.path.join(os.path.dirname(OCTOPUS_TILT_PATH), f"octopus_aligned_{method}.jpg")
        ...
        try:
            ...
            # Check that the output file exists
            assert os.path.exists(result), f"Output file not created for method {method}: {result}"
            
            # Check that the output file is not empty
            assert os.path.getsize(result) > 0, f"Output file is empty for method {method}"
...
def test_align_image_angle_threshold():
    ...
    for threshold in thresholds:
        # Create a temporary output path
        output_path = os.path.join(os.path.dirname(OCTOPUS_TILT_PATH), f"octopus_aligned_threshold_{threshold}.jpg")
        
        try:
            # Run alignment with the current threshold
            result = align_image(OCTOPUS_TILT_PATH, output_path, angle_threshold=threshold)
            
            # Check that the output file exists
            assert os.path.exists(result), f"Output file not created for threshold {threshold}: {result}"
            
            # Check that the output file is not empty
            assert os.path.getsize(result) > 0, f"Output file is empty for threshold {threshold}"
...
def test_detect_angle_functions():
    ...
    print(f"Detected angles - Hough: {angle_hough:.2f}°, Contour: {angle_contour:.2f}°, Center: {angle_center:.2f}°")
...
def test_filename_generation():
    ...
    try:
        result = align_image(OCTOPUS_TILT_PATH, method=method, angle_threshold=threshold)
        
        # Check that the output filename contains the method and threshold
        expected_suffix = f"_aligned_{method}_a{threshold:.1f}.jpg"
        assert expected_suffix in result, f"Output filename {result} does not contain expected suffix {expected_suffix}"
