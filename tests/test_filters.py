...
def test_list_filters():
    ...
    expected_filters = [
        "grayscale", "sepia", "blur", "sharpen", "brightness", 
        "contrast", "saturation", "vignette", "edge_enhance"
    ]
...
def test_cli_multiple_filters():
    ...
    # Run CLI command
    cmd = ["imagewand", "filter", TEST_IMAGE, "-f", "grayscale,sharpen", "-o", output_path]
    result = subprocess.run(cmd, capture_output=True, text=True)
...
def test_cli_filter_with_parameters():
    ...
    # Run CLI command
    cmd = ["imagewand", "filter", TEST_IMAGE, "-f", "contrast:factor=1.2", "-o", output_path]
    result = subprocess.run(cmd, capture_output=True, text=True)
