...
def test_autocrop_modes():
    ...
    for mode in modes:
        ...
        assert os.path.exists(result), f"Output file not created for mode {mode}: {result}"
        
        # Verify the output filename contains the mode
        if mode == "auto":
            assert "_auto" in result or "_auto_frame" in result or "_auto_border" in result
        elif mode == "frame":
...
def test_crop_framed_photo():
    ...
    assert output_img.shape != input_img.shape, "Image dimensions should change after cropping"
