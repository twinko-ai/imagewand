...
class AutoMerge:
    ...
    def _try_all_orientations(self, img1: np.ndarray, img2: np.ndarray) -> Tuple[np.ndarray, np.ndarray, List[cv2.DMatch]]:
        """
    ...
    def merge_images(self, images: List[np.ndarray]) -> np.ndarray:
        ...
        for img in images:
            ...
            if self.debug:
                print(f"Original shape: {img.shape}, After border removal: {processed.shape}")
        ...
        for i in range(1, len(processed_images)):
            ...
            kp1, oriented_img2, matches = self._try_all_orientations(result, processed_images[i])
